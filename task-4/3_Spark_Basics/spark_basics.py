...
spark = SparkSession.builder.appName("spark_basic").getOrCreate()


df_people = (
    spark.read.option("header", "true")
    .schema(people_schema)
    .csv("data/top_100_richest.csv")
)

df_country = (
    spark.read.option("header", "true")
    .schema(country_schema)
    .csv("data/wiki_number_of_billionaires.csv")
)


df_people_filtered = df_people.withColumn(
    "net_worth", regexp_extract(col("net_worth"), "^\$(\\d+).*$", 1).cast("int")
).filter(col("net_worth") > 60)

df_people_new = df_people_filtered.withColumn(
    "nationality",
...
df_prelast = df_people_new.join(
    df_country, df_people_new["nationality"] == df_country["country"], "inner"
)

df_last = df_prelast.filter(col("age").isNotNull()).select(
    col("rank"),
    col("name"),
    col("net_worth"),
    col("bday"),
...
plans_1 = df_last._sc._jvm.PythonSQLUtils.explainString(
    df_last._jdf.queryExecution(), "EXTENDED"
)
